# These functions fit exactly one analysis pipeline per simulated data set.
#
# The pipeline is deliberately simple:
# 1) optionally residualise X and Y using one chosen residualiser
# 2) fit one chosen SEM to the resulting data
#
...
# 
# The following functions are specificied:
# - helpers: logistics
# - helpers: fit a lavaan model without hard errors
# - wrapper: residualise data by calling the chosen residualiser
# - wrapper: fit the chosen SEM to the (optionally) residualised data
# -------------------------------------------------------------------------------------------------
...
fit_analysis_pipeline <- function(
  ...
  # fit the SEM safely
  fit_out <- safe_fit_lavaan(model_string = model_string, data = df_stage2)
