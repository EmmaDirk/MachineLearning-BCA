...
# RW mean vector for each coefficient (overtime trend in effects)
RW_mean <- matrix(0, nrow = 2, ncol = k)                   # here: zero trend
...
# For t = 1, there is no carryover; target: S_target = S_U + Sigma_e1
Sigma_e1 <- S_dyn
...
# write.csv(df, file = "Thesis/Code/Data/Data_Sim_tanh_RW_Pop.csv", row.names = FALSE)
