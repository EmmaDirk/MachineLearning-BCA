...
# curvature parameter: larger = quicker "turning" around a point
a_tanh <- 2

# simulate base confounders: C1..C3 ~ N(0,1), independent
Psi_base <- diag(rep(1, k_lin))                            # variance-covariance matrix of base confounders (I)
U_lin <- rmvnorm(N, 
                 mean  = rep(0, k_lin),                    # mean of confounders = 0
...
# T_j = standardized tanh(a_tanh * C_j), with linear part removed
...
# order in gamma_x / gamma_y corresponds to (C1, C2, C3, T1, T2, T3)
...
# If B_list exists (list of length T, each 2 x k), use it.
...
# For t = 1, no carryover; target: S_target = S_U + Sigma_e1
Sigma_e1 <- S_dyn
...
# first wave: add direct confounder effects using B_list[[1]]
...
# write.csv(df, file = "Thesis/Code/Data/Data_Sim_tanh_Blist_Pop.csv", row.names = FALSE)
