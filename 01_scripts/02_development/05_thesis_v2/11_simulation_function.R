...
#
# The original single-model interface is preserved as a wrapper.
# -------------------------------------------------------------------------------------------------
...
# validate every model specification against the old single-model rules
...
run_simulation_study <- function(
  ...
  # wrap the old single-model call into the new model-set interface
  ...
  # keep the legacy result shape by dropping the helper column
  results_legacy <- out$results[, setdiff(names(out$results), "model_name"), drop = FALSE]
  ...
  list(
    ...
    study = list(
      ...
      bootstrap_B = bootstrap_B,
      bootstrap_seed = bootstrap_seed,
      cores = out$study$cores,
      base_seed = base_seed,
      xgb_tuning = out$study$model_specs[[1]]$xgb_tuning,
      tune_xgb = tune_xgb,
      xgb_tune_args = xgb_tune_args,
      residualizer_args = residualizer_args,
      model_specs = out$study$model_specs,
      stage1_groups = out$study$stage1_groups
    )
  )
}
